        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            @if($briefs->isEmpty())
                <div class="flex h-48 flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
                    ...
                    @if($search)
                        <button wire:click="$set('search', '')" class="mt-2 text-xs text-brand-600 hover:underline dark:text-brand-400">Clear search</button>
                    @endif
                </div>
            @else
                <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($briefs as $brief)
                        <div class="group rounded-2xl border border-zinc-200 bg-white p-6 transition hover:border-brand-300 hover:shadow-md dark:border-zinc-700 dark:bg-zinc-900 dark:hover:border-brand-700">
                            <div class="mb-3 flex items-center justify-between">
                                <span class="inline-flex items-center gap-1 rounded-full bg-brand-100 px-2.5 py-0.5 text-xs font-medium text-brand-700 dark:bg-brand-900/30 dark:text-brand-400">
                            ...
                            <h3 class="mb-3 font-semibold text-zinc-900 transition group-hover:text-brand-700 dark:text-white dark:group-hover:text-brand-400">{{ $brief->title }}</h3>
                            @if($brief->body)
                                ...
                                <tr class="transition hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl
                                                {{ $ext === 'PDF' ? 'bg-crimson-50 text-crimson-700 dark:bg-crimson-800/20 dark:text-crimson-300' :
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <a href="{{ $doc->url() }}" target="_blank" download
                                            class="inline-flex items-center gap-1.5 rounded-xl border border-brand-200 bg-brand-50 px-3 py-1.5 text-xs font-medium text-brand-700 transition hover:bg-brand-100 dark:border-brand-800 dark:bg-brand-900/20 dark:text-brand-400 dark:hover:bg-brand-900/40">

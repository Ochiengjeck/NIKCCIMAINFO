<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <flux:subheading>Manage blog posts, categories and comments</flux:subheading>
        </div>
        <div class="flex items-center gap-2">
            <flux:button :href="route('admin.cms.blog.categories')" wire:navigate variant="ghost" icon="tag">Categories</flux:button>
            <flux:button :href="route('admin.cms.blog.comments')" wire:navigate variant="ghost" icon="chat-bubble-left-right">Comments</flux:button>
            @can('cms.edit')
                <flux:button wire:click="openForm()" variant="primary" icon="plus">New Post</flux:button>
            @endcan
        </div>
    </div>
    ...
    @if($showForm)
        <div class="mb-6 rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:heading size="lg" class="mb-5">{{ $editingId ? 'Edit Post' : 'New Post' }}</flux:heading>
            ...
            <form wire:submit="save" class="space-y-5">
                <div class="grid gap-4 lg:grid-cols-2">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <flux:field>
                            ...
                            <flux:select wire:model="blogCategoryId" placeholder="Uncategorised">
                                <flux:select.option value="">Uncategorised</flux:select.option>
                                @foreach($categories as $category)
                                    <flux:select.option value="{{ $category->id }}">{{ $category->name }}</flux:select.option>
                                @endforeach
                            </flux:select>
                            <flux:error name="blogCategoryId" />
                        </flux:field>
                ...
                <flux:field>
                    ...
                    <flux:textarea wire:model="body" rows="12" placeholder="Full post content (HTML supported)" />
    ...
    <div class="mb-4 flex flex-wrap gap-3">
        <flux:input wire:model.live.debounce.300ms="search" placeholder="Search posts..." icon="magnifying-glass" class="max-w-xs" />
    ...
    <div class="overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                    ...
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <div>
                                    <p class="text-sm font-medium text-zinc-900 dark:text-white">{{ $post->title }}</p>
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium
                                {{ $post->status === 'published' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-zinc-100 text-zinc-600 dark:bg-zinc-700 dark:text-zinc-400' }}">
                                {{ ucfirst($post->status) }}
                            </span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex justify-end gap-2">
                                @can('cms.edit')
                                    <flux:button wire:click="openForm({{ $post->id }})" size="sm" variant="ghost" icon="pencil" />
                                    @if($post->status === 'draft')
                                        <flux:button wire:click="publish({{ $post->id }})" size="sm" variant="ghost">Publish</flux:button>
                                    @else
                                        <flux:button wire:click="unpublish({{ $post->id }})" size="sm" variant="ghost">Draft</flux:button>
                                    @endif
                                    <flux:button wire:click="delete({{ $post->id }})" wire:confirm="Delete this post?" size="sm" variant="ghost" icon="trash" class="text-red-500" />
                                @endcan
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-10 text-center text-sm text-zinc-500">No posts found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

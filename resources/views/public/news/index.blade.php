<x-layouts::website
    ...
    <section class="relative overflow-hidden py-32 text-white">
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
            ...
            <p class="max-w-2xl mx-auto text-lg text-zinc-300">News, press releases and announcements from NiKCCIMA and the AfCFTA trade corridor.</p>
        </div>
    </section>
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            @if($activeCategory || $activeTag)
                <div class="mb-10 flex items-center gap-3 text-sm">
                    <a href="{{ route('news.index') }}" class="font-medium text-brand-700 hover:underline">All news</a>
            ...
            <div class="grid grid-cols-1 gap-12 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2">
                    @if($articles->isEmpty())
                        <div class="flex h-64 flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-300 bg-zinc-50">
                            ...
                            <p class="text-sm text-zinc-500">No news published yet. Check back soon.</p>
                        </div>
                    @else
                                ...
                                <article class="rounded bg-white shadow-[0_0_15px_rgba(0,0,0,0.1)] hover:shadow-lg transition-all duration-500 overflow-hidden flex flex-col">
                                    ...
                                    <div class="p-6 flex flex-col flex-1">
                                        <div class="flex items-center gap-3 mb-3">
                                            @if($article->category)
                                                <a href="{{ route('news.category', $article->category->slug) }}" class="text-xs font-semibold uppercase tracking-widest text-crimson-700 bg-crimson-50 px-2 py-0.5 rounded hover:bg-crimson-100">{{ $article->category->name }}</a>
                                            @endif
                                        ...
                                        <div class="mt-5 flex items-center justify-between">
                                            <a href="{{ route('news.show', $article->slug) }}" class="text-sm font-medium text-brand-600 hover:text-crimson-700 transition-colors">Read More &rarr;</a>
                ...
                <aside class="space-y-10">
                    ...
                    @if($recent->isNotEmpty())
                        <div>
                            <h3 class="mb-4 font-serif text-lg font-bold text-zinc-900">Recent News</h3>
                            <ul class="space-y-3">
                                @foreach($recent as $post)
                                    <li>
                                        <a href="{{ route('news.show', $post->slug) }}" class="group block">
                    ...
                    <div>
                        <a href="{{ route('news.feed') }}" class="inline-flex items-center gap-2 text-sm font-medium text-brand-700 hover:text-crimson-700">

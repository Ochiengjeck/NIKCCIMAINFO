<x-layouts::website :title="'News & Insights — NiKCCIMA'">
    ...
    <section class="relative overflow-hidden py-24 text-white">
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <span class="mb-4 inline-flex rounded-full bg-[#922529] px-4 py-1.5 text-xs font-semibold uppercase tracking-widest text-white">Press</span>
            <h1 class="font-serif text-4xl font-bold lg:text-6xl">News & Insights</h1>
            <p class="mt-4 max-w-2xl text-lg text-[#922529]">Latest press releases, trade outcomes, policy insights, and institutional updates from NiKCCIMA.</p>
        </div>
    </section>
                ...
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($articles as $article)
                        <a href="{{ route('news.show', $article->slug) }}"
                           class="group flex flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white transition hover:-translate-y-0.5 hover:shadow-lg">
                            ...
                            @else
                                <div class="flex h-52 items-center justify-center bg-gradient-to-br from-brand-50 to-zinc-100">
                            ...
                            <div class="flex flex-1 flex-col p-6">
                                ...
                                <h2 class="mt-2 line-clamp-2 font-semibold text-zinc-900 transition group-hover:text-[#922529]">
                                    {{ $article->title }}
                                </h2>

                                @if($article->excerpt)
                                    <p class="mt-2 line-clamp-3 text-sm text-zinc-500">{{ $article->excerpt }}</p>
                                @endif

                                <p class="mt-3 text-sm font-medium text-[#922529]">Read More &rarr;</p>
                            </div>
                        </a>
                    @endforeach
                </div>

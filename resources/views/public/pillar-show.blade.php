...
<x-layouts::website :title="($page?->title ?? ($pillars[$pillarSlug] ?? 'Pillar')).' — NiKCCIMA'">
    ...
    <section class="relative overflow-hidden py-20 text-white">
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            {{-- Breadcrumb --}}
            <nav class="mb-5 flex items-center gap-2 text-sm text-green-300" aria-label="Breadcrumb">
                ...
                <span class="text-green-100">{{ $page?->title ?? ($pillars[$pillarSlug] ?? 'Pillar Details') }}</span>
            </nav>

            <span class="mb-4 inline-flex rounded-full bg-red-600 px-4 py-1.5 text-xs font-semibold uppercase tracking-widest text-white">Pillar</span>
                ...
                <aside class="shrink-0 lg:w-56 lg:sticky lg:top-24 lg:self-start">
                        ...
                        @foreach($pillars as $slug => $name)
                            <a href="{{ route('pillars.show', $slug) }}"
                               class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors duration-150
                                ...
                                @if($pillarSlug === $slug)
                                    <span class="h-1.5 w-1.5 rounded-full bg-green-600 dark:bg-green-400 shrink-0"></span>
                                @else
                    ...
                    <div class="mt-6 hidden lg:block">
                        <a href="{{ route('pillars') }}"
                           class="inline-flex items-center gap-1.5 text-xs font-medium text-zinc-400 hover:text-green-600 dark:hover:text-green-400 transition-colors duration-150">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    @if($page)
                        ...
                        @if($page->section('mandate'))
                            <div class="mb-8 rounded-2xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 p-8">
                                <div class="flex items-start gap-4">
                                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-green-700 text-white mt-0.5">
                                    ...
                                    <div class="flex-1">
                                        <div class="mb-3 flex items-center gap-3">
                                            <span class="rounded-full bg-red-600 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-white">Mandate</span>
                                        </div>
                                        <p class="text-green-900 dark:text-green-100 leading-relaxed text-base">
                                ...
                                <div class="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 shadow-sm">
                                    <div class="mb-5 flex items-center gap-3">
                                        <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-green-100 dark:bg-green-900/40">
                                            <svg class="h-4.5 w-4.5 h-5 w-5 text-green-700 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                ...
                                <div class="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 shadow-sm">
                                    <div class="mb-5 flex items-center gap-3">
                                        <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-100 dark:bg-emerald-900/40">
                                            <svg class="h-5 w-5 text-emerald-700 dark:text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    @else
                        ...
                        <div class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 py-20 px-8 text-center">
                            ...
                            <a href="{{ route('contact') }}"
                               class="mt-6 inline-flex items-center gap-2 rounded-full bg-green-700 px-5 py-2.5 text-sm font-semibold text-white hover:bg-green-800 transition-colors duration-200">
                                Contact Us
                            </a>
                        </div>
                    @endif

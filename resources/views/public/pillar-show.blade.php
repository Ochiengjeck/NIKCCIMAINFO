...
<x-layouts::website :title="($page?->title ?? ($pillars[$pillarSlug] ?? 'Pillar')).' — NiKCCIMA'">
    ...
    <section class="relative overflow-hidden py-20 text-white">
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            {{-- Breadcrumb --}}
            <nav class="mb-5 flex items-center gap-2 text-sm text-[#922529]" aria-label="Breadcrumb">
                <a href="{{ route('pillars') }}" class="hover:text-white transition-colors duration-150">Pillars</a>
                <svg class="h-3.5 w-3.5 text-[#922529]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                </svg>
                <span class="text-[#922529]">{{ $page?->title ?? ($pillars[$pillarSlug] ?? 'Pillar Details') }}</span>
            </nav>

            <span class="mb-4 inline-flex rounded-full bg-[#922529] px-4 py-1.5 text-xs font-semibold uppercase tracking-widest text-white">Pillar</span>
                ...
                <aside class="shrink-0 lg:w-56 lg:sticky lg:top-24 lg:self-start">
                        ...
                        @foreach($pillars as $slug => $name)
                            <a href="{{ route('pillars.show', $slug) }}"
                               class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors duration-150
                                      {{ $pillarSlug === $slug
                                          ? 'bg-[#A8DCAB]/20 text-[#922529] font-semibold'
                                          : 'text-zinc-600 hover:bg-white hover:text-zinc-900' }}">
                                @if($pillarSlug === $slug)
                                    <span class="h-1.5 w-1.5 rounded-full bg-[#922529] shrink-0"></span>
                                @else
                    ...
                    <div class="mt-6 hidden lg:block">
                        <a href="{{ route('pillars') }}"
                           class="inline-flex items-center gap-1.5 text-xs font-medium text-zinc-400 hover:text-[#922529] transition-colors duration-150">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    @if($page)
                        ...
                        @if($page->section('mandate'))
                            <div class="mb-8 rounded-2xl border border-[#A8DCAB]/30 bg-[#A8DCAB]/20 p-8">
                                <div class="flex items-start gap-4">
                                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#922529] text-white mt-0.5">
                                    ...
                                    <div class="flex-1">
                                        <div class="mb-3 flex items-center gap-3">
                                            <span class="rounded-full bg-[#922529] px-3 py-1 text-xs font-semibold uppercase tracking-wider text-white">Mandate</span>
                                        </div>
                                        <p class="text-[#922529] leading-relaxed text-base">
                            ...
                            @if($page->section('responsibilities'))
                                <div class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
                                    <div class="mb-5 flex items-center gap-3">
                                        <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-[#A8DCAB]/20">
                                            <svg class="h-4.5 w-4.5 h-5 w-5 text-[#922529]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    ...
                                    <ul class="space-y-3">
                                        @foreach(array_filter(explode("\n", $page->section('responsibilities'))) as $item)
                                            <li class="flex items-start gap-3 text-sm text-zinc-600 leading-relaxed">
                                                <svg class="mt-0.5 h-4 w-4 shrink-0 text-[#922529]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                                </svg>
                                                {{ trim($item) }}
                                            </li>
                    ...
                    @else
                        ...
                        <div class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-300 bg-white py-20 px-8 text-center">
                            ...
                            <a href="{{ route('contact') }}"
                               class="mt-6 inline-flex items-center gap-2 rounded-full bg-[#922529] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#922529] transition-colors duration-200">
                                Contact Us
                            </a>
                        </div>
                    @endif
                    ...
                    <div class="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-zinc-200 pt-6">
                        <a href="{{ route('pillars') }}"
                           class="inline-flex items-center gap-2 rounded-lg border border-zinc-200 bg-white px-4 py-2.5 text-sm font-medium text-zinc-700 hover:border-[#A8DCAB]/30 hover:text-[#922529] transition-colors duration-150">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($pillars as $slug => $name)
                                @if($slug !== $pillarSlug)
                                    <a href="{{ route('pillars.show', $slug) }}"
                                       class="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-xs font-medium text-zinc-500 hover:border-[#A8DCAB]/30 hover:text-[#922529] transition-colors duration-150">
                                        {{ $name }}
                                        <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                                        </svg>

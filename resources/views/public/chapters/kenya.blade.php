...
<x-layouts::website :title="'Kenya Chapter — NiKCCIMA'">
    ...
    <section class="relative overflow-hidden py-24 text-white">
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <span class="mb-4 inline-flex items-center gap-2 rounded-full bg-[#922529] px-4 py-1.5 text-xs font-semibold uppercase tracking-widest text-white">🇰🇪 Kenya</span>
            <h1 class="font-serif text-4xl font-bold lg:text-6xl">{{ $page?->section('heading', 'Kenya Chapter') }}</h1>
            @if($page?->section('description'))
                <p class="mt-4 max-w-2xl text-lg text-[#922529]">{{ $page->section('description') }}</p>
            @endif
        </div>
    </section>
        ...
        <section class="py-16 bg-white">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="grid items-center gap-12 lg:grid-cols-2">
                    <div>
                        <span class="text-xs font-semibold uppercase tracking-widest text-[#922529]">🇰🇪 Kenya Chapter</span>
                    ...
                    <div class="relative">
                        ...
                        />
                        <div class="absolute -bottom-4 -right-4 -z-10 h-full w-full rounded-2xl border-2 border-[#922529]"></div>
                    </div>
                </div>
            </div>
        </section>
            ...
            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                <div class="rounded-2xl border border-zinc-200 bg-zinc-50 p-6">
                    <div class="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-[#A8DCAB]/20">
                        <svg class="h-5 w-5 text-[#922529]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    </div>
                    <p class="mb-1 text-xs font-semibold uppercase tracking-widest text-zinc-400">Address</p>
                    <p class="text-sm text-zinc-700">{{ $page?->section('address', 'Nairobi, Republic of Kenya') }}</p>
                </div>
                <div class="rounded-2xl border border-zinc-200 bg-zinc-50 p-6">
                    <div class="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-[#A8DCAB]/20">
                        <svg class="h-5 w-5 text-[#922529]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                    </div>
                    <p class="mb-1 text-xs font-semibold uppercase tracking-widest text-zinc-400">Email</p>
                    <a href="mailto:{{ $page?->section('email', 'kenya@nikccima.org') }}" class="text-sm text-[#922529] hover:underline">{{ $page?->section('email', 'kenya@nikccima.org') }}</a>
                </div>
                @if($page?->section('phone'))
                    <div class="rounded-2xl border border-zinc-200 bg-zinc-50 p-6">
                        ...
                        <p class="text-sm text-zinc-700">{{ $page->section('phone') }}</p>
                    </div>
                @endif
            </div>
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach(array_filter(explode(',', $page->section('initiatives'))) as $initiative)
                        <span class="rounded-full border border-[#922529] bg-[#A8DCAB]/20 px-4 py-1.5 text-sm font-medium text-[#922529]">{{ trim($initiative) }}</span>
                    @endforeach
                </div>
            </div>
        </section>
    ...
    @if($profiles->isNotEmpty())
        <section class="bg-white py-20">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="mb-10">
                    <span class="mb-2 inline-block text-xs font-semibold uppercase tracking-widest text-[#922529]">People</span>
                ...
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    @foreach($profiles as $profile)
                        <div class="rounded-2xl border border-zinc-200 bg-white p-6 text-center transition hover:shadow-md">
                            ...
                            @if($profile->email)
                                <a href="mailto:{{ $profile->email }}" class="mt-3 inline-block text-xs text-[#922529] hover:underline">{{ $profile->email }}</a>
                            @endif
                        </div>
                    @endforeach
                </div>
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($events as $event)
                        <a href="{{ route('events.show', $event->id) }}" class="group rounded-2xl border border-zinc-200 bg-white p-6 transition hover:border-[#922529] hover:shadow-md">
                            <p class="mb-2 text-xs font-medium text-[#922529]">{{ $event->starts_at->format('d M Y') }}</p>
                            <h3 class="font-semibold text-zinc-900 transition group-hover:text-[#922529] line-clamp-2">{{ $event->title }}</h3>
                            @if($event->venue)
    ...
    {{-- CTA --}}
    <section class="bg-[#922529] py-14 text-white">
        <div class="mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8">
            <h2 class="font-serif text-2xl font-bold">Connect with the Kenya Chapter</h2>
            <p class="mt-3 text-[#922529]">Get in touch with our Nairobi office for trade inquiries, membership, or chapter events.</p>
            <a href="{{ route('contact') }}" class="mt-6 inline-flex items-center gap-2 rounded-xl bg-white px-7 py-3 text-sm font-semibold text-[#922529] transition hover:bg-[#7a1e22]">
                Contact Us

...
<x-layouts::website :title="'Nigeria Chapter — NiKCCIMA'">
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="grid items-center gap-12 lg:grid-cols-2">
                    ...
                    <div class="order-1 lg:order-2">
                        <span class="text-xs font-semibold uppercase tracking-widest text-brand-600">🇳🇬 Nigeria Chapter</span>
    ...
    @if($page?->section('initiatives'))
        <section class="py-12 bg-zinc-50">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="mb-6">
                    <span class="mb-2 inline-block text-xs font-semibold uppercase tracking-widest text-brand-600">
                        Active Programs
                    </span>
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach(array_filter(explode(',', $page->section('initiatives'))) as $initiative)
                        <span class="rounded-full border border-brand-200 bg-brand-50 px-4 py-1.5 text-sm font-medium text-brand-700">
                            {{ trim($initiative) }}
                        </span>
                    @endforeach
                </div>
                        ...
                        <div class="rounded-2xl border border-zinc-200 bg-white p-6 text-center transition hover:-translate-y-0.5 hover:shadow-md">
                            @if($profile->photoUrl())
                                <img src="{{ $profile->photoUrl() }}"
                                     alt="{{ $profile->name }}"
                                     class="mx-auto mb-4 h-20 w-20 rounded-full object-cover ring-4 ring-brand-200">
                            @else
                                <div class="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-brand-700 to-brand-400 ring-4 ring-brand-200">
                            ...
                            <p class="mt-1 text-sm text-brand-700">{{ $profile->position }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8 flex items-end justify-between">
                    ...
                    <a href="{{ route('events.index') }}"
                       class="text-sm font-medium text-brand-700 transition hover:text-brand-800">
                        All Events &rarr;
                    </a>
                </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($events as $event)
                        <a href="{{ route('events.show', $event->id) }}"
                           class="group rounded-2xl border border-zinc-200 bg-white p-5 transition hover:border-brand-300 hover:shadow-md">
                            <p class="mb-1.5 text-xs font-medium text-zinc-400">
                                {{ $event->starts_at->format('d M Y') }}
                            </p>
                            <h3 class="font-semibold text-zinc-900 line-clamp-2 transition group-hover:text-brand-700">
                                {{ $event->title }}
                            </h3>
                            @if($event->venue)

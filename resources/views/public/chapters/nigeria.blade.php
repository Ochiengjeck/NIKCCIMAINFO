...
<x-layouts::website :title="'Nigeria Chapter — NiKCCIMA'">
    ...
    <section class="relative overflow-hidden py-24 text-white lg:py-32">
        ...
        @if($page?->section('banner_image'))
            <div class="absolute inset-0">
                <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($page->section('banner_image')) }}"
                     ...
                     aria-hidden="true">
                <div class="absolute inset-0 bg-gradient-to-br from-brand-950/90 via-brand-900/80 to-brand-700/70"></div>
            </div>
        @else
            <div class="absolute inset-0 bg-gradient-to-br from-brand-950 via-brand-900 to-brand-700"></div>
        @endif
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="max-w-3xl">
                <span class="mb-4 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-brand-200">
                    &#127475;&#127468; Nigeria
                </span>
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="grid items-center gap-12 lg:grid-cols-2">
                    ...
                    <div class="order-1 lg:order-2">
                        <span class="text-xs font-semibold uppercase tracking-widest text-brand-600 dark:text-brand-400">🇳🇬 Nigeria Chapter</span>
                        ...
                        <div class="mt-6 flex flex-wrap gap-3">
                            <a href="{{ route('contact') }}" class="inline-flex items-center gap-2 rounded-xl bg-crimson-700 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-crimson-800">
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-6 sm:grid-cols-3">
                ...
                <div class="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
                    <div class="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-brand-50 dark:bg-brand-950/40">
                        <svg class="h-5 w-5 text-brand-600 dark:text-brand-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <div class="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
                    ...
                    <a href="mailto:{{ $page?->section('email', 'nigeria@nikccima.org') }}"
                       class="text-sm font-medium text-brand-700 transition hover:text-brand-800 hover:underline dark:text-brand-400 dark:hover:text-brand-300">
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach(array_filter(explode(',', $page->section('initiatives'))) as $initiative)
                        <span class="rounded-full border border-brand-200 bg-brand-50 px-4 py-1.5 text-sm font-medium text-brand-700 dark:border-brand-800 dark:bg-brand-900/20 dark:text-brand-400">
                            {{ trim($initiative) }}
                        </span>
                    @endforeach
                </div>
                        ...
                        <div class="rounded-2xl border border-zinc-200 bg-white p-6 text-center transition hover:-translate-y-0.5 hover:shadow-md dark:border-zinc-700 dark:bg-zinc-900">
                            @if($profile->photoUrl())
                                <img src="{{ $profile->photoUrl() }}"
                                     alt="{{ $profile->name }}"
                                     class="mx-auto mb-4 h-20 w-20 rounded-full object-cover ring-4 ring-brand-200 dark:ring-brand-900">
                            @else
                                <div class="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-brand-700 to-brand-400 ring-4 ring-brand-200 dark:ring-brand-900">
                            ...
                            <p class="mt-1 text-sm text-brand-700 dark:text-brand-400">{{ $profile->position }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8 flex items-end justify-between">
                    ...
                    <a href="{{ route('events.index') }}"
                       class="text-sm font-medium text-brand-700 transition hover:text-brand-800 dark:text-brand-400 dark:hover:text-brand-300">
                        All Events &rarr;
                    </a>
                </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($events as $event)
                        <a href="{{ route('events.show', $event->id) }}"
                           class="group rounded-2xl border border-zinc-200 bg-white p-5 transition hover:border-brand-300 hover:shadow-md dark:border-zinc-700 dark:bg-zinc-800/60 dark:hover:border-brand-700">
                            <p class="mb-1.5 text-xs font-medium text-zinc-400 dark:text-zinc-500">
                                {{ $event->starts_at->format('d M Y') }}
                            </p>
                            <h3 class="font-semibold text-zinc-900 line-clamp-2 transition group-hover:text-brand-700 dark:text-white dark:group-hover:text-brand-400">
                                {{ $event->title }}
                            </h3>
                            @if($event->venue)
    ...
    {{-- ===================== FOOTER CTA ===================== --}}
    <section class="relative overflow-hidden py-16 bg-brand-900 text-white">
        <svg class="absolute inset-0 h-full w-full opacity-10" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <defs>
        ...
        <div class="relative mx-auto max-w-3xl px-4 text-center sm:px-6 lg:px-8">
            ...
            <p class="mb-8 text-brand-200">
                Reach out to our Abuja office to learn more about membership, trade programmes, and upcoming events.
            </p>
            <a href="{{ route('contact') }}"
               class="inline-flex items-center gap-2 rounded-xl bg-white px-8 py-3.5 text-sm font-bold text-brand-900 transition hover:bg-brand-50 hover:shadow-md">
                Get in Touch

    ...
    <section class="py-16 bg-white">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            ...
            <nav class="mb-8 flex items-center gap-2 text-sm text-zinc-400">
                <a href="{{ route('blog.index') }}" class="font-medium text-brand-700 hover:underline">Blog</a>
            ...
            <div class="mb-4 flex flex-wrap items-center gap-3">
                @if($article->category)
                    <a href="{{ route('blog.category', $article->category->slug) }}" class="inline-flex rounded-full bg-brand-100 px-3 py-1 text-xs font-medium text-brand-700 hover:bg-brand-200">
            ...
            @if($article->tags->isNotEmpty())
                <div class="mt-8 flex flex-wrap gap-2">
                    @foreach($article->tags as $tag)
                        <a href="{{ route('blog.tag', $tag->slug) }}" class="rounded-full bg-zinc-100 px-3 py-1 text-xs text-zinc-600 hover:bg-zinc-200">#{{ $tag->name }}</a>
                    @endforeach
                </div>
            @endif
            ...
            <div class="mt-12 border-t border-zinc-200 pt-10">
                ...
                <livewire:public.blog-comment-form :post-id="$article->id" :key="'comment-form-'.$article->id" />
            </div>

            {{-- Related Posts --}}
            @if($related->isNotEmpty())
                <div class="mt-16 border-t border-zinc-200 pt-10">
                    <h2 class="mb-6 font-serif text-2xl font-bold text-zinc-900">Related Posts</h2>
            ...
            <div class="mt-12 border-t border-zinc-200 pt-8">
                <a href="{{ route('blog.index') }}" class="inline-flex items-center gap-2 text-sm font-medium text-brand-700 hover:text-brand-800 transition">

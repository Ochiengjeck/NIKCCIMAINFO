<x-layouts::website :title="'Apply for Membership — NiKCCIMA'">
    ...
    <section class="relative overflow-hidden py-20 text-white">
        <div class="absolute inset-0 bg-gradient-to-br from-[#922529] to-[#922529]">
        ...
        <div class="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <nav class="mb-5 flex items-center gap-2 text-sm text-[#922529]">
            ...
            <span class="mb-4 inline-flex rounded-full bg-[#922529] px-4 py-1.5 text-xs font-semibold uppercase tracking-widest text-white">Application</span>
            <h1 class="font-serif text-4xl font-bold lg:text-5xl">Membership Application</h1>
            <p class="mt-4 max-w-xl text-[#922529]">Complete the form below to apply for NiKCCIMA membership. Our team reviews all applications within 5 working days.</p>

            {{-- Steps indicator --}}
            <div class="mt-8 flex items-center gap-3">
                @foreach(['Personal Info', 'Category', 'Business Profile', 'Declaration'] as $i => $label)
                    <div class="flex items-center gap-2">
                        <div class="flex h-6 w-6 items-center justify-center rounded-full {{ $i === 0 ? 'bg-white text-[#922529]' : 'bg-[#922529]/60 text-[#922529]' }} text-xs font-bold">{{ $i + 1 }}</div>
                        <span class="hidden text-xs text-[#922529] sm:block">{{ $label }}</span>
                    </div>
                    @if($i < 3)
                        <div class="h-px w-6 bg-[#922529]/50"></div>
                    @endif
                @endforeach
            </div>
        </div>

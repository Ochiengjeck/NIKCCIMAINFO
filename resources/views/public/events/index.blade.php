            ...
            @if($upcoming->isEmpty())
                <div class="flex h-48 items-center justify-center rounded-2xl border border-dashed border-zinc-300 bg-zinc-50">
                    <div class="text-center">
                        ...
                        <p class="text-zinc-500">No upcoming events scheduled. Check back soon.</p>
                    </div>
                </div>
            @else
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($upcoming as $event)
                        @php
                            ...
                            $stripColor = $isFlagship ? 'bg-[#922529]' : ($isMission ? 'bg-[#922529]' : 'bg-[#922529]');
                            $badgeColor = $isFlagship ? 'bg-[#922529]' : 'bg-[#922529]';
                        @endphp
                        <a href="{{ route('events.show', $event->id) }}"
                           class="group overflow-hidden rounded-2xl border border-zinc-200 bg-white transition hover:-translate-y-0.5 hover:shadow-lg">
                            ...
                            <div class="p-6">
                                ...
                                <div class="mb-3 flex items-end gap-2">
                                    <span class="font-serif text-4xl font-bold leading-none text-zinc-900">
                                        {{ $event->starts_at->format('d') }}
                                    </span>
                                    <span class="mb-0.5 text-xs font-semibold uppercase tracking-widest text-zinc-400">
                                ...
                                {{-- Title --}}
                                <h3 class="mt-2 font-semibold leading-snug text-zinc-900 line-clamp-2 transition group-hover:text-[#922529]">
                                    {{ $event->title }}
                                </h3>
    ...
    @if($past->isNotEmpty())
        <section class="py-16 bg-zinc-50">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8">
                    <span class="mb-2 inline-block text-xs font-semibold uppercase tracking-widest text-zinc-400">
                        Archive
                    </span>
                    <h2 class="font-serif text-2xl font-bold text-zinc-700">
                        Past Events
                    </h2>
                </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($past as $event)
                        <a href="{{ route('events.show', $event->id) }}"
                           class="group rounded-2xl border border-zinc-200 bg-white p-5 transition hover:shadow-sm">
                            ...
                            <h3 class="text-sm font-semibold text-zinc-600 line-clamp-2 transition group-hover:text-zinc-800">
                                {{ $event->title }}
                            </h3>
                            @if($event->venue)

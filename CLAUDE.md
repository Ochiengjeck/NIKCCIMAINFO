...
### Public Website & CMS
Public pages are rendered through `PublicController` + the `layouts.website` layout (Blade `<x-layouts::website>`); a full-page Livewire component uses `->layout('layouts.website', [...])`. Content is editable via the admin CMS module (`app/Livewire/Cms/` — pages, blog, news, leadership, media, contact). CMS/public models (`CmsPage`, `BlogPost`/`BlogCategory`/`BlogTag`/`BlogComment`, `NewsArticle`/`NewsCategory`/`NewsTag`/`NewsComment`, `LeadershipProfile`, `MediaItem`, `ContactInquiry`) deliberately do **not** use `ChapterScoped`.

### Blog & News (two parallel modules)
**Blog** and **News** are independent, near-identical modules — Blog (`/blog`, `blog_*` tables) and News (`/news`, `news_*` tables). Each has an admin manager + category manager + comment moderator (`app/Livewire/Cms/{Blog,News}Manager.php`, `{Blog,News}CategoryManager`, `{Blog,News}CommentModerator`), a `{Blog,News}Post`/`Article` model with `*Category`/`*Tag` taxonomy and moderated `*Comment`s, and a public surface: `/{blog,news}` (listing + sidebar), `/{blog,news}/{slug}` (post + related + author bio + comments), `/{blog,news}/category/{slug}`, `/{blog,news}/tag/{slug}`, and an RSS feed at `/{blog,news}/feed`. Comments default to `pending` and only show once approved via the moderator (gated by `cms.publish`). Seed default categories with `db:seed --class=BlogCategorySeeder` and `--class=NewsCategorySeeder`. Note: News was originally renamed into Blog, so the recreated `news_articles` table uses explicitly-named constraints (`news_articles_slug_unq`, `news_articles_author_fk`, `news_articles_category_fk`) to avoid colliding with the names `blog_posts` inherited from that rename.
